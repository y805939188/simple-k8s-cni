...
/**
 * # 查看主机上 ebpf 程序注册的 map
 * bpftool map list --bpffs -j | jq
 * 
 * # 查看 id === 94 的 map
 * bpftool map dump id 94
 * 
 * cilium 就存了一堆这玩意儿
 * 比如叫做 “cilium_tunnel_map” 的 map
 * 里头存的是集群中, 除本机以外的节点对应的网段和节点 ip 的映射
 * 
 * 再比如叫做 “cilium_lxc” 的 map
 * 存的就是 pod 们的 ip 和两张 veth 网卡的 mac 的映射（用 ifindex 做的 key）
 * 
 */
...
int cls_main(struct __sk_buff *skb) {
  ...
  // skb->cb[CB_IFINDEX];
  // int ifindex = ctx_load_meta(skb, CB_IFINDEX);
